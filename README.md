...
- or with the standard subprotocol trick: the client offers two subprotocols `Sec-WebSocket-Protocol: token, <the token>`. The gateway extracts the value following `token`, validates it, and rewrites the header to `Sec-WebSocket-Protocol: token` so it still answers the client's handshake with the negotiated `token` subprotocol (required by some browsers, e.g. Chrome). This subprotocol is then stripped entirely before the connection is proxied to the downstream service, since the raw token must never reach it and the downstream service doesn't need to (and generally doesn't) support subprotocol negotiation at all.
